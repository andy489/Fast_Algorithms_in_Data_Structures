...
void update_y(int vx, int lx, int rx, int vy, int ly, int ry, int x, int y, int newVal) {
    if (ly == ry) {
        ...
        int my = (ly + ry) >> 1;
        if (y <= my)
            update_y(vx, lx, rx, vy * 2, ly, my, x, y, newVal);
        else
            update_y(vx, lx, rx, vy * 2 + 1, my + 1, ry, x, y, newVal);
        t[vx][vy] = t[vx][vy * 2] + t[vx][vy * 2 + 1];
    }
}
...
void update_x(int vx, int lx, int rx, int x, int y, int newVal) {
    if (lx != rx) {
        int mx = (lx + rx) >> 1;
        if (x <= mx)
            update_x(vx * 2, lx, mx, x, y, newVal);
        else
            update_x(vx * 2 + 1, mx + 1, rx, x, y, newVal);
    }
    update_y(vx, lx, rx, 1, 0, m - 1, x, y, newVal);
}
